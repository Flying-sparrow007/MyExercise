...
public class UserDaoImpl implements UserDao {
	...
	public int updateUserData(User user) {
		...
		try {
			conn = DButil.getConnection();
			String sql = "update usertable set username = ?,useremail = ?,userphone = ? where id = ?";
			ps = conn.prepareStatement(sql);
			ps.setString(1, user.getUserName());
	...
	/**根据分页查询用户数据*/
	@Override
	public List<User> findUserByPage(int start, int rowSize) {
		Connection conn = null;
		PreparedStatement ps = null;
		ResultSet rs = null;
		List<User> list = null;
		try {
			list = new ArrayList<User>();
			conn = DButil.getConnection();
			String sql = "select id,username,password,useremail,userphone from usertable limit ?, ?";
			ps = conn.prepareStatement(sql);
			ps.setInt(1, start);
			ps.setInt(2, rowSize);
			rs = ps.executeQuery();
