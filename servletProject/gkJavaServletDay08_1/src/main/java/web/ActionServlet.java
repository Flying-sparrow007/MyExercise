...
public class ActionServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		}else if(uri.endsWith("nextPage.do")){
			//处理下一页数据
			nextPageServletAction(request, response);
		}else if(uri.endsWith("upPage.do")){
			//处理上一页数据
			upPageServletAction(request, response);
		}else if(uri.endsWith("userPage.do")){
			//处理具体页数数据
			userPageServletAction(request, response);
		}
	}

	//处理具体数据
	private void userPageServletAction(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		//获取当前页数
		...
		//获取当前页的用户数据
		List<User> list = dao.findUserByPage(start, rowSize);
	...
	//处理上一页数据
	private void upPageServletAction(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		//获取当前页数
		int page = Integer.parseInt(request.getParameter("page"));
		...
		//获取总页数
		int pageSize = Integer.parseInt(request.getSession().getAttribute("pageSize").toString());
		//判断当 当前页数小于等于1则页数为当前页数,即第一页,否则页数减少1
		if(page <= 1){
			page = 1;
		}else{
			page--;
	...
	//处理下一页数据
	private void nextPageServletAction(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		//获取当前页数
		int page = Integer.parseInt(request.getParameter("page"));
		UserDao dao = new UserDaoImpl();
		//总页数
		int pageSize = Integer.parseInt(request.getSession().getAttribute("pageSize").toString());
		//判断当 当前页数大于等于总页数则页数为当前页数,即最后一页,否则页数增加1
		if(page >= pageSize){
			page = pageSize;
		}else{
			page++;
	...
	private void findUserServletAction(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try {
			...
			//1.根据分页查询用户数据
			int start = 0;//默认起始位置1
			int rowSize = 2;//每一页显示的行数
			List<User> list = dao.findUserByPage(start, rowSize);
			
			//2.获取当前页数
			int page = 1;//登录成功后默认显示第一页
			
			//3.获取总共页数
			int countRow = dao.selectUserCountAll();//先获取总行数
			int pageSize = countRow % rowSize == 0 ? countRow / rowSize : countRow / rowSize + 1;//在获取总页数
	...
	private void addUserServletAction(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try {
			...
			if(userPwd == null || userPwd.trim().isEmpty()){
				request.setAttribute("megPwd", "用户密码不能为空!");
				//获取转发器并转发
				request.getRequestDispatcher("addUser.jsp").forward(request, response);
				return ;
			}
