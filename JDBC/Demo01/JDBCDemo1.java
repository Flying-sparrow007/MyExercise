 ...
 * 一套JDBC接口的实现,打成一个jar包发布,各个包叫驱动包.
 * 
 * JDBC接口中提供了:
...
public class JDBCDemo1 {
	public static void main(String[] args) {
		...
		try {
			 ...
			 * int executeUpdate(String sql)(负责数据库插入,修改,删除)
			 * 若返回1则执行成功,返回0则执行失败,它是专门用来执行DML语句
			 * 
			 * ResultSet executeQuery(String sql)(查询)专门来执行DQL语句,返回查询的结果集,
			 * 用ResultSet实例返回
			 * 
			 * boolean execute(String sql)(创建)可以执行任何类型的SQL语句,但由于DML和DQL都是专门
			 * 的方法执行,所以该方法一般用来执行DDL,返回值为执行后是否有结果
			 */
			//用Statement执行SQL语句
			state.execute(sql);
